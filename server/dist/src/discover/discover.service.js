...
let DiscoverService = class DiscoverService {
    ...
    async getDiscoverFeed(userId, gameId, limit = 10) {
        ...
        const filtered = hasPremiumFilters
            ? candidates.filter((c) => {
                if (myProfile.filterGender) {
                    ...
                    if (c.gender && c.gender !== wanted)
                        return false;
                }
                if (c.dateOfBirth && (myProfile.filterAgeMin != null || myProfile.filterAgeMax != null)) {
                    const age = this.getAge(c.dateOfBirth);
                    if (myProfile.filterAgeMin != null && age < myProfile.filterAgeMin)
                        return false;
                    if (myProfile.filterAgeMax != null && myProfile.filterAgeMax < 50 && age > myProfile.filterAgeMax)
                        return false;
                }
                if (myProfile.filterMicOnly && !c.usesMic)
                    return false;
                if (myProfile.filterPlayStyles && myProfile.filterPlayStyles.length > 0) {
                    ...
                    if (c.playStyle && !mapped.includes(c.playStyle))
                        return false;
                }
                return true;
            })
        ...
        const scored = filtered.map((candidate) => {
            ...
            const sharedGames = myGameIds.filter((id) => candidateGameIds.includes(id));
            let score = 20;
            if (sharedGames.length > 0) {
                score += Math.min(sharedGames.length * 12, 35);
            }
            if (gameId && sharedGames.includes(gameId))
                score += 15;
